...
sub anagram
{
    ...
    $word = util::orderWord( $word );     # canonize input.
    my( @rez ) = anagrams( $word ); # find anagrams.
    @rez = sort( @rez );            # return in alpha order.
...
sub countGrams
{
    ...
    my( $sorted ) = util::orderWord( $word );
    my( @letters ) = util::explode( $sorted );
